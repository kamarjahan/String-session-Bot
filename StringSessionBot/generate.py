...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nPlease Report to @devourdevils if there is an error " \
...
async def main(_, msg):
    await msg.reply(
        "Please click Which String You Want To Take",
...
async def generate_session(bot, msg, telethon=False):
    ...
    phone_number_msg = await bot.ask(user_id, 'Now Send `PHONE_NUMBER` with contry code. \nExample : `+918330039856`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    ...
    try:
        ...
        return
    try:
        phone_code_msg = await bot.ask(user_id, "OTP sent successfully Please check for an OTP in official telegram account. If you got it, send it here this format. \nIf OTP is `12345`, space one number  **like** `1 2 3 4 5`.", filters=filters.text, timeout=600)
        if await cancelled(api_id_msg):
            return
    except TimeoutError:
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by [devourdevils](t.me/devourdevils) @septemberfilms".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully fetching session string {}. \n\nPlease check on Saved Messages! \n\nBy @devourdevils".format("telethon" if telethon else "pyrogram"))
...
#     if "/cancel" in msg.text:
#         await msg.reply(" Processes!", quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
#         return True
#     elif "/restart" in msg.text:
